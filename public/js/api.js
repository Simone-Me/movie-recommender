class MovieApi {
    static async searchMovies(query = '', year = '', genre = '') {
        ...
        if (genre?.toString().trim()) params.append('genre', genre.toString().trim());
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (form) {
        form.addEventListener('submit', async function(e) {
            ...
            if (!query && !year && !genre) {
                resultsContainer.innerHTML = '<div class="alert alert-warning">Veuillez entrer au moins un critère de recherche (titre, année ou genre).</div>';
                return;
            }
            ...
            try {
                ...
                console.error('Error fetching movies:', error);
                resultsContainer.innerHTML = '<div class="alert alert-danger">Une erreur est survenue lors de la recherche des films.</div>';
            }
        });
    }
    ...
    function updateResults(movies) {
        ...
        if (!movies || !movies.length) {
            resultsContainer.innerHTML = '<div class="alert alert-info">Aucun film trouvé avec ces critères.</div>';
            return;
        }
        ...
        resultsContainer.innerHTML = movies.map(movie => `
            <div class="col-md-4 mb-4">
                <div class="card h-100">
                    ...
                    <div class="card-body">
                        <h5 class="card-title">${movie.title || 'Sans titre'}</h5>
                        <p class="card-text">${movie.overview ? movie.overview.substring(0, 150) + '...' : 'Aucune description disponible'}</p>
                        <div class="d-flex justify-content-between align-items-center">
                            <span class="badge bg-primary">${movie.voteAverage || '?'}/10</span>
                            <a href="/movie/${movie.id}" class="btn btn-sm btn-outline-primary">Voir plus</a>
                        </div>
                    </div>
                </div>
            </div>
